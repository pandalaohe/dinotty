...
| `mocha-theme` | no | n/a | n/a | private | n/a | | never — fork identity (excluded from #148/#149 by design). **Known defect, open and unscheduled (found `2026-09-10`, T260910-006):** the settings theme list renders this theme's raw i18n key `settings.theme.catppuccin-mocha` instead of a label, because the theme was added (`e86d122`, `2026-08-24`, `frontend/src/themes.ts:549`) without a matching label key and `t()` falls back to `table[key] ?? key`. NOT a regression of any align — `themeLabel` and that fallback are byte-identical before and after the `2026-09-10` merge; it has been visible since the theme shipped. Fix is two i18n keys (EN + ZH) per this repo's bilingual convention; the `.ts` write routes through the code-writer lane. No board row holds this — this row is where it lives, so whoever next touches the Mocha theme picks it up here |
...
| `upstream-only-toolbar-inset-under-open-kb` | yes | https://github.com/xichan96/dinotty/pull/275 | | merged-upstream | absorbed | upstream-pr/toolbar-inset-under-open-keyboard | met `2026-08-27` — PR #275 MERGED as `beb9e30`, now the `upstream/dev` tip; verified verbatim (`git diff upstream-pr/toolbar-inset-under-open-keyboard beb9e30` empty on both touched files, so the maintainer changed nothing). Pulled into `custom` by the `2026-08-27` alignment merge, where its expression was DECLINED in favour of this fork's flat `8px` — see `5e7ecb3-toolbar-safe-area-divergence`, whose PR #277 asks upstream to adopt the flat form. **Carries NO fork tree presence and never will** — it is the upstream-shaped half of `5e7ecb3-toolbar-safe-area-divergence`, authored directly on `upstream/dev` and deliberately not carried here, because this fork's own flat `8px` already makes it a no-op. Upstream reserves `env(safe-area-inset-bottom)` as `#system-mobile-kb`'s bottom padding unconditionally, but the toolbar is fixed to the KEYBOARD edge, not the screen — while the native IME is up the home indicator is under the keyboard, so the reserved strip lands where nothing can occupy it, and it is not free: the toolbar's measured height feeds `--mkb-height`, which `#app-root` subtracts. Filed as `max(8px, calc(env(safe-area-inset-bottom, 0px) - var(--sys-kb-height, 0px)))` — a strict no-op with the keyboard down and anywhere the inset is already 0. The `var()` fallback is load-bearing, not defensive: `useViewportResize`'s teardown calls `removeProperty` rather than setting `0px`, and an unresolvable `var()` with no fallback invalidates the whole declaration at computed-value time. **Disclosed in the PR body and repeated here: this exact expression has never run on a device** — what was verified on the user's iPhone was the fork-local flat variant. Task `T260825-009`, board `KB12` |
| `5e7ecb3-toolbar-safe-area-divergence` | yes | https://github.com/xichan96/dinotty/pull/277 | | merged-upstream | absorbed | upstream-pr/toolbar-inset-flat-when-keyboard-down | met `2026-08-27` — PR #277 MERGED upstream as `f69c73f`; upstream adopted this fork's flat `8px` verbatim, so the divergence this row existed to hold is GONE. Absorbed into `custom` by the `2026-09-10` alignment merge (`247719e`), which resolved the last `mobile-keyboard.css` / `SystemKeyboardToolbar.test.ts` conflict toward upstream on that ground: the fork's own comment said to drop the note once the #275 follow-up landed, and `f69c73f` IS that follow-up. Both files now match `upstream/dev` byte-for-byte, so neither conflicts on a future align — which was the whole point of filing. Original filing rationale follows as provenance. **Until `2026-08-27` this mod existed ONLY as narrative prose under the Live-alignment residual layer and had no row here** — precisely the gap this Index exists to close ("a mod with no row is a mod a rebuild deletes silently"); the row is added now so a future rebuild cannot drop it unseen. `#system-mobile-kb`'s bottom pad is a flat `8px` where upstream — even after #275 — still carries `max(8px, calc(env(safe-area-inset-bottom, 0px) - var(--sys-kb-height, 0px)))`, which with the keyboard DOWN reduces to `max(8px, env(safe-area-inset-bottom))`, i.e. 34px on current iPhones. Why that is not free space: `useKeyboardBand` measures the toolbar's BORDER-BOX height into `--mkb-height` (`useKeyboardBand.ts:31`) and `#app-root` subtracts it (`App.vue:794`), so in `system_toolbar_mode: 'persistent_mobile'` — where the bar stays visible with the keyboard down (`useAppCore.ts:138`) — the inset is charged to terminal height for the WHOLE session, ~26px on an 812pt iPhone. In the DEFAULT toolbar mode the bar unmounts on keyboard close, `--mkb-height` is 0, and the reservation costs nothing; the divergence therefore bites only in persistent mode, and that scoping is stated in the PR. **Reclassified `private` → `filed` on `2026-08-27`**: the earlier "stays private" verdict was written before #275 landed, when the alternative was a permanent hand-resolved conflict at every `align-upstream`; filing upstream removes that recurring cost, and if #277 is rejected the row reverts to `private` with the conflict resolution recorded rather than rediscovered. Trade-off disclosed in the PR body and here: the page dots (`.system-kb-page-dots`, real `<button>`s) drop from ~42px to ~8px above the screen edge. Accepted on the user's own iPhone (installed home-screen app, 8998, `2026-08-27`) — dots comfortable to hit, no misfire, no complaint. Task `T260826-016`, board `KB14` |
| `plugin-float-window-localstorage-stub` | yes | https://github.com/xichan96/dinotty/pull/296 | | merged-upstream | absorbed | fix/float-window-localstorage-stub | met `2026-09-10` — PR #296 MERGED upstream as `7b758f7c`; the four test files are identical to `upstream/dev` and left the fork residual on the `2026-09-13` align (26 → 22 paths). **Root cause corrected `2026-09-13` (T260913-TMB-CC-020): the analysis below is wrong about WHY `localStorage` is missing.** It is not a property of this repo's vitest + happy-dom setup: Node 25+ enables an experimental global `localStorage` by default (flag since 22.4) that shadows happy-dom's inside vitest workers (local Node v26.3.0; upstream CI runs Node 20). Proof: the 7 upstream test files added after `6c97227` fail 59 tests on pristine `upstream/dev@34a5e19b` and pass with `NODE_OPTIONS=--no-experimental-webstorage` (full suite 1706 passed / 0 failed). The merged stubs are harmless; the general fix is row `vitest-node-webstorage-off`. Original filing rationale follows as provenance: met when PR #296 is merged or rejected. The four plugin float-window test files upstream added in `2a50023` (`2026-09-05`) call bare `localStorage` with no stub, and under this repo's vitest + happy-dom setup `localStorage` is not exposed as a bare global — the property lives on happy-dom's `Window` but never reaches `globalThis`. All 17 cases therefore error before reaching an assertion, RED on a pristine `upstream/dev@6c97227` worktree with none of this fork's changes present (verified `2026-09-10` by running those four files in a detached upstream worktree; the failing-case set `comm -23` against the post-merge run is EMPTY, which is what proved the red merge-neutral). Fork commit `da6b722` adds the repo's OWN minimal module-scope `MemoryStorage` + `vi.stubGlobal('localStorage', …)` to each, matching the 11 existing sites under `frontend/src/test/` (`OverlayDragItem.test.ts:44`, `useSyncWebSocketPluginWorkspace.test.ts:26` are the shape copied). Not only about the `beforeEach` cleanup call: `PluginFloatWindow.vue:142` reads `localStorage.getItem(storageKey.value)` while restoring saved geometry, so the stub is what lets `mount()` run at all. Filed as #296 from a branch cut from `upstream/dev@6c97227`, never from `custom`; `infra-git-hygiene.sh check --pre-pr --base upstream/dev .` CLEAN before push. **Why this one was filed when the `2026-08-27` disposition dropped its sibling**: same defect class as item 2 there (`OverlayDragItem.test.ts` fixture, dropped as not-specifically-tested), different files and a different cost — `scripts/dinotty-ops.sh`'s frontend verify is a HARD gate with no skip switch, so an upstream-caused red vitest blocks EVERY `dinotty rebuild` on this fork until it is patched. Carrying the stub locally is therefore not optional; filing it upstream is what stops it being carried forever. If #296 is rejected this row reverts to `private` and the four paths stay a permanent local carry; if merged, the fork residual drops by 4 paths. Task `T260910-006`, board `UPA01` |
| `vitest-node-webstorage-off` | no | n/a | n/a | private | n/a | | met when this Mac's Node is below 25 or vitest/happy-dom stop letting Node's experimental global `localStorage` shadow happy-dom's — then drop the prefix. Node 25+ enables that global by default (flag since 22.4); inside vitest workers it shadows happy-dom's `localStorage`, so any upstream test using bare `localStorage` fails locally while upstream CI (Node 20) stays green. On the `2026-09-13` align that was 7 upstream-new test files / 59 tests, identical on pristine `upstream/dev@34a5e19b`, and `dinotty rebuild`'s vitest gate refused to build. Fix (`81e6ba13`, fork-owned files only, user decision D2): `NODE_OPTIONS=--no-experimental-webstorage` on the two vitest calls in `scripts/dinotty-ops.sh` (`frontend_verify_gate`, `align_upstream` verify) and on `.upstream-update.json` `verify.web-test` + `survival.mobile-input` (`env` argv prefix). A hand-run `pnpm exec vitest run` without the variable still shows the false red. Expected on a Node that does not know the flag: a loud failure, not a silent pass — Node rejects an unknown `NODE_OPTIONS` flag with exit 9 (`is not allowed in NODE_OPTIONS`; observed on Node 26 with a made-up flag, Node 20 itself not run). Task `T260913-TMB-CC-020` |
...
- Filed `2026-08-26` (T260825-009 / board `KB12`): **four upstream PRs opened, three lifecycle flips `candidate`→`filed` plus one new upstream-only row.** `ff419d4-viewport-pan-lock` → [#272](https://github.com/xichan96/dinotty/pull/272); `b238998-plugin-store-instance-isolation` → [#273](https://github.com/xichan96/dinotty/pull/273); `1f0cd07-builtin-kb-seed-scoping` → [#274](https://github.com/xichan96/dinotty/pull/274); `upstream-only-toolbar-inset-under-open-kb` → [#275](https://github.com/xichan96/dinotty/pull/275). Every branch was cut from `upstream/dev@8b644d9`, never from `custom`, and every one passed `infra-git-hygiene.sh check --pre-pr --base upstream/dev .` CLEAN before push. **Two facts worth carrying, because both cost time this run.** (1) `upstream/dev` is RED on its own: `frontend/src/test/OverlayDragItem.test.ts` fails 15 tests on a clean checkout of `8b644d9` with none of our changes present. Every PR body says so explicitly so the maintainer does not attribute it to us — and the fork's own copy of that file is GREEN, so the fork holds a fix upstream lacks. That is a fifth PR candidate, recorded here and deliberately not scheduled — and DROPPED `2026-08-27` by user decision (see the `2026-08-27` disposition entry). (2) Files restored from tag `backup/custom-20260824` were never prettier-formatted, so `useViewportPanLock.ts` and its test failed upstream's `prettier --check` on first push; they were formatted on the PR branch only. **The fork's own copies on `custom` are still unformatted** — a follow-up owed here, not upstream. Per-PR verification at filing: #272 vitest 1316/15, #273 `cargo test` 618/0 + fmt + clippy clean, #274 vitest 1331/15 + `npm run build` green, #275 vitest 1308/15 — every 15 being the pre-existing `OverlayDragItem` set above, zero failures outside it on any branch.
- Filed + aligned `2026-08-27` (T260826-016 / board `KB14`): **PR [#277](https://github.com/xichan96/dinotty/pull/277) opened, and `custom` re-aligned onto `upstream/dev@beb9e30`.** #277 is the CLOSED-keyboard half of #275: it asks upstream to adopt this fork's flat `8px` bottom pad so the two trees stop diverging on `mobile-keyboard.css` / `SystemKeyboardToolbar.test.ts` and `align-upstream` stops needing a hand-resolved conflict every time. Branch cut from `upstream/dev@beb9e30`, never from `custom`; `infra-git-hygiene.sh check --pre-pr --base upstream/dev .` CLEAN before push, re-run CLEAN on the committed state. **Three facts worth carrying.** (1) All four `2026-08-26` PRs merged VERBATIM — `git diff <pr-branch> <merge-commit>` empty on every touched file, so the maintainer changed nothing. Only #275 conflicted on re-align, and only because it was authored directly on `upstream/dev` with no fork tree presence, while the fork had changed the same line differently; the other three were cherry-picks OF fork code and 3-way-merged silently. (2) The align merge is a behavioural NO-OP for the running app: built bundles are byte-identical (`index-C_RdScP3.js` + `index-jkdu1Amu.css` both in local `dist/` and served by 8998), because the sole content delta was a CSS COMMENT and comments are stripped in the production build. 8998 needed no rebuild to test the result — worth remembering before scheduling one. (3) `align-upstream`'s flaky-test carve-out is DEAD CODE: `cargo test terminal_exit_regression` matches zero tests (613 filtered out), so the `--skip` in its step 2 and the whole special-case handling in step 3 are no-ops. Recorded here, deliberately not scheduled. Post-merge verification was run BY HAND, because `align-upstream` early-returns on `behind==0` and therefore silently skips its ENTIRE verification suite after any manual conflict resolution — a trap worth stating plainly: `cargo check --workspace` exit 0, `src-tauri` crate check exit 0, `cargo test --workspace` 0 failures, `vue-tsc --noEmit` 0 errors, `vitest run` **146/147 files passed, 0 failed, 1360/1370 tests**. Note the 15 `OverlayDragItem.test.ts` failures that are RED on `upstream/dev` are GREEN here — re-confirming the fork holds a fix upstream lacks. That fifth PR candidate was **DROPPED `2026-08-27` by user decision**; see the disposition entry immediately below.
- Disposition `2026-08-27` (T260825-004 / board `RCB01`): **three items dropped by user decision during the KB board reconciliation.** Recorded here because a dropped candidate keeps its exit condition at the workaround site (P8 official-first), and because three other ledger sites above would otherwise keep re-seeding these as open work.
  1. **SSH session close on write failure** — dropped. User's stated ground: this fork has never exercised SSH ("我们貌似没搞过 ssh"). Consequence carried: upstream still leaves an unusable tab on an SSH write failure, and this fork no longer carries the dispatcher that closed it either (removed by the `2026-08-24` rebuild), so the behaviour is absent on BOTH sides. Nothing tracks it; re-open by filing against upstream's own per-connection `input_tx` model.
...
- Residual fork layer vs `upstream/dev`: **22 paths, 3984 insertions / 26 deletions** (`git diff --stat
  ...
  - `5e7ecb3-toolbar-safe-area-divergence` (`2026-08-26`, `KB12`/`T260825-009`, accepted on device) —
    ...
    after physical-device QA. **SUPERSEDED `2026-08-27`: this is no longer private — it now has its own Contribution
    Index row and is FILED upstream as PR #277.** The text below is kept as provenance for why it was
    private first. Giving up the closed-state
    home-indicator clearance is the fork owner's choice, re-confirmed on device `2026-08-27`, and the lower key row
    now sits at the edge of the home-gesture region. The IME-OPEN half is a genuine upstream defect — the
    keyboard covers the indicator, so reserving its inset is dead space by construction — and is
    filed upstream as PR #275 in its own right (row `upstream-only-toolbar-inset-under-open-kb`), expressed as an `.ime-open` override
...
- Two upstream-PR candidates opened BY the rebuild: (a) SSH session close on write
  ...
  copy of that test file does not carry. **Both DROPPED `2026-08-27` by user decision — see the
  `2026-08-27` disposition entry below for the reason and the consequence.** Neither was ever filed, and
  neither was ever reproduced on clean upstream (which filing would have required first).
...
- Re-align log (newest first):
    ...
    - `2026-09-10` (T260910-006 / board `UPA01`) → base `6c97227` (was `beb9e30`), custom HEAD `da6b722`,
      ...
      **the largest align since the `2026-08-24` rebuild — 52 upstream commits, version 0.23.1 → 0.25.1.**
      Run through the repo's own `bash scripts/dinotty-ops.sh align-upstream --yes`. `merge-tree` predicted
      the conflict surface exactly: 2 conflicted files (`frontend/src/styles/mobile-keyboard.css` 604-620,
      comment block only; `frontend/src/test/SystemKeyboardToolbar.test.ts` 86-104), 2 auto-merged
      (`AboutTab.vue`, `src/settings/types/mod.rs`). Both conflicts resolved toward UPSTREAM, and for a
      stated reason rather than a preference: `f69c73f` (PR #277, this fork's own) landed upstream on
      `2026-08-27`, which is precisely the follow-up our comment named as its own deletion condition.
      Both files now match upstream byte-for-byte.
      Loss check, verification, deployment, blind spots and the one unscheduled pre-existing defect are in
      the `2026-09-10` snapshot entry above; the two things a future align most needs from it are (1) never
      whole-file-take `mobile-keyboard.css` — the untested iOS-26 `.ime-open` block sits just past the
      conflict region — and (2) a fork deletion OUTSIDE a conflict region auto-merges silently and can strand
      the other side's in-conflict code (`const toolbarPadding` did exactly that here).
      New local mod out of this run: `plugin-float-window-localstorage-stub` (`da6b722`), filed upstream as
      PR #296 — see its Index row.
...
- **Detach-reap redesign: reference-based session liveness + crash-orphan boot sweep** (2026-07-23;
  ...
  `/Volumes/Dev/ai/core/docs/task-files/dinotty_mods/26/07/260723_T260723-003_detach-reap-redesign_design.md` (rev2; repointed `2026-08-27` — this one was broken by the 2026-08-04 FT2 migration itself, not by the RCB01 move, and note the layout changed from `2607/` to `26/07/`).
...
`/Volumes/Dev/ai/core/docs/task-files/dinotty_mods/26/08/260812_T260812-014_verification.md` (repointed `2026-08-27`).
