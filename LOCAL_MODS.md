...
| `epv1-preview-freeze-annotate` | maybe | | | candidate | pending | | open — v1 freeze/annotate/export on kind=web pane. r2 (both-surface): proxy injects an in-page capture bridge (`src/proxy/inject.rs` + `mod.rs` + `external.rs`) so the previewed page rasterizes itself and returns an ImageBitmap over a MessagePort — works on packaged Tauri (cross-origin iframe) AND browser. Frontend: WebPreview.vue + WebAnnotationLayer.vue + previewImage.ts + preview-bridge/index.ts + previewAnnotationRetention.ts + vite bridge entry + i18n; snapdom dep. Committed `e5778984`. Old parent-side snapDocument/captureViewport path retained as P6 fallback until Tauri-surface trial passes. upstream PR candidacy decided post-E2E per design T260723-028 |
| `qkb3-ime-keyboard-overlap` | yes | https://github.com/xichan96/dinotty/pull/216 | | pr-open | pending | upstream-pr/ime-keyboard-overlap-grow | open — PR #216 merged upstream → absorbed (branch cut from `upstream/dev` @ `9cbb9d5d` which already carries QKB2; clean cherry-pick of custom `d8f70121`+`efa48641`+`5e50967d` squashed to `d846007d`; vue-tsc + vitest 865/865 + vite build green on upstream base, hygiene --pre-pr CLEAN, 0 trailer) |
