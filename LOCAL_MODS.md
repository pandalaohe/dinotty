...
| `kb01-kb06-mobile-ime-closure` | maybe | | | candidate | pending | codex/kb-series-20260810 | open — complete frozen 8998/native QA and real-iPhone Simplified Pinyin + WeChat IME acceptance, then split upstreamable slices |
...
| `system-keyboard-toolbar-customization` | yes | https://github.com/xichan96/dinotty/pull/254 | | merged-upstream | absorbed | codex/upstream-system-keyboard-customization | met `2026-08-14` — PR #254 closed/conflicting, but upstream independently absorbed the same toolbar/settings behavior as `990e1bc7`; later touch/IME closure is PR #256 |
...
| `touch-web-terminal-selection-guard` | yes | https://github.com/xichan96/dinotty/pull/261 | | filed | pending | codex/upstream-touch-web-selection-20260818-r2 | filed `2026-08-18` as PR #261 from fresh `upstream/dev@7a99861b`; head `b57b8242`. The two-file patch removes only the touch-Web `preventDefault()` focus guard that blocks native terminal text selection under `open_only`/`both`; manual IME opening remains governed by the xterm helper textarea policy. Real touch-Web acceptance passed on 8998; PR branch `check-pr`, infra hygiene, frontend full tests/build, and `cargo check --workspace` passed |
...
> **Reconciled against `upstream/dev@7a99861b` on `2026-08-18`.** The integration candidate now contains
> complete upstream v0.22.0 history including merged PR #257–#260 and the keyboard provider/host-bridge
> refactor, plus only the still-deliberate fork/configuration layer. Official implementations
> win whenever they satisfy the recorded intent; the older local copies are not stacked on top.
> Historical narrative below is provenance and may describe files that have since moved or been absorbed.
- Residual fork layer vs `upstream/dev`: 55 paths, 6663 insertions / 639 deletions. It consists of:
  - private fork identity and operations: Mocha theme, macOS signing identity, `.upstream-update.json`,
    `LOCAL_MODS.md`, `.gitignore`, `scripts/dinotty*`, its relaunch regression, and the input replay diagnostic;
  - user-retained behavior: session input dispatch/failure handling and its tests, terminal/PTY/SSH/openapi
    integration needed by that path, keyboard toggle translation, and the transport warning;
  - no residual notification implementation, Windows resolver, plugin-tab persistence, multiline quick-send,
    plugin pane identity/visibility, or preview-loopback-auth patch: those are now upstream-owned.
