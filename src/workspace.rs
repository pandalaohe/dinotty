...
fn parse_python_diagnostics(stderr: &str, content: &str) -> Vec<SyntaxDiagnostic> {
    // python3 error format: File "<stdin>", line N, ...
    // or:   File "<stdin>", line N\n    ...\nSyntaxError: ...
    ...
    while let Some(line) = lines.next() {
        if let Some(rest) = line.strip_prefix("  File \"<stdin>\", line ") {
            ...
            // Consume caret line and error type line
...
fn parse_go_diagnostics(stderr: &str, content: &str) -> Vec<SyntaxDiagnostic> {
    // go tool compile: file.go:N:M: message
    let mut diags = Vec::new();
...
fn check_go_syntax(content: &str) -> Vec<SyntaxDiagnostic> {
    use std::process::{Command, Stdio};
    // Write to temp file so go tool compile can find it
    let tmp = match tempfile::NamedTempFile::new() {
        Ok(f) => f,
        Err(_) => return vec![],
    };
    let tmp_path = tmp.path().with_extension("go");
